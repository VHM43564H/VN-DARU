...
![DOI](https://zenodo.org/badge/DOI/10.5281/zenodo.21359334.svg)
...
How to run
Open the notebook in Google Colab (badge above, or upload `VN_DARU_colab.ipynb` to colab.research.google.com).
Install dependencies in the first cell:
```
   !pip install -q yfinance google-cloud-bigquery db-dtypes statsmodels
   ```
Part 1 only: set `PROJECT_ID` to your own Google Cloud project ID (BigQuery API enabled). GDELT is a free public dataset; queries are billed against your project's BigQuery quota.
Run Part 1 to generate `VN_DARU_weekly_all_variants.csv` and `VN_DARU_merged_with_RV.csv` in your Google Drive (`MyDrive/VN_DARU/`).
Run Part 2 to reproduce all tables and figures reported in the paper.
Data availability
...
```
Citation metadata is also available in `CITATION.cff` (use GitHub's "Cite this repository" button).
License
Code is released under the MIT License. Figures © the authors, reproduced here for replication purposes.
Contact
Corresponding author: Huynh Tan Toi — huynhtantoi.law@gmail.com — ORCID 0009-0008-2542-6645
---
Hướng dẫn nhanh (tiếng Việt)
Kho lưu trữ này chứa mã nguồn tái lập cho bài báo về chỉ số VN-DARU — chỉ số bất định pháp lý tài sản số cho Việt Nam. Notebook `VN_DARU_colab.ipynb` gồm 2 phần: Phần 1 thu thập dữ liệu GDELT qua BigQuery và dựng chỉ số (cần Project ID Google Cloud của bạn); Phần 2 phân tích, kiểm định và vẽ lại Hình 1–2 từ file CSV do Phần 1 tạo ra. Mở trực tiếp trên Google Colab bằng nút "Open in Colab" ở đầu trang.
